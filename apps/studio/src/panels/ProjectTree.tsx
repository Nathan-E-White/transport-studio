...
export function ProjectTree({project, selectedEntityId, onSelect, stats}: ProjectTreeProps) {
    ...
    return <section className="panel project-panel">
        ...
        {groups.map((group) => {
            ...
            return <div className="tree-group" key={group.kind}>
...
function describeEntity(entity: SceneEntity) {
    switch (entity.kind) {
        case "geometry":
            return `${entity.primitive} · ${entity.materialId ? "material assigned" : "no material"}`;
        case "material":
            return `μ=${entity.attenuationCoefficient.toFixed(2)} · scatter ${Math.round(entity.scatterProbability * 100)}%`;
        case "source":
            return `${entity.sourceKind} · ${entity.particleType} · ${entity.energy} MeV`;
        case "tally":
            return `${entity.tallyKind} · ${entity.particleTypes.join(", ")}`;
    }
}
